...
// `sanitizeExperiment` valida tipo e faixa de cada chave da config vinda do
// localStorage (`__irisflowExp.set(...)`): um cast `as Partial<ExperimentConfig>`
// deixaria `l2csCadenceMs: 0` (throttle desligado) ou `expandFactor: "x"` (NaN,
// crop degenerado + warn por frame) entrarem intactos.

afterEach(() => {
  vi.restoreAllMocks();
});

describe('sanitize rejeita tipo errado', () => {
  ...
  it('número onde deveria ser booleano cai no default', () => {
    const r = sanitizeExperiment({ geometricPoseCompensation: 1 as unknown as boolean });
    expect(r.geometricPoseCompensation).toBe(true);
  });
...
describe('sanitize aplica faixa por chave', () => {
  ...
  it('`l2csInputSize` recusa valores que não são múltiplos de 32', () => {
    // O risco concreto numa sessão de medição é um dedo trocado: `244` em vez
    // de `224`. Uma faixa `{min:224,max:448}` o aceitaria e a condição mediria
    // uma terceira coisa — sem erro, sem aviso.
  ...
  it('booleanos válidos passam', () => {
    const r = sanitizeExperiment({ geometricPoseCompensation: false, polynomialFeatures: false });
    expect(r.geometricPoseCompensation).toBe(false);
  ...
  it('um valor inválido não contamina os outros', () => {
    ...
    expect(r.geometricPoseCompensation).toBe(false); // preservado
  });
