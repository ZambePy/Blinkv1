...
import baselineJson from '../../docs/baseline_a28bdb0.json';
import { EXPERIMENT } from '../config/experiment';

// -----------------------------------------------------------------------------
// P7.6 — integração ponta a ponta no harness.
//
// ── O aceite pedido, e por que ele não podia ser satisfeito ─────────────────
//
// O plano diz: *"o harness roda com `filterMode: 'kalmanEma'`,
// `featureSet: 'spec11'`, CLAHE/gama/ROI ligados, e todas as métricas ficam
// dentro da tolerância declarada em `docs/baseline_a28bdb0.json`"*.
//
...
describe('o portão: o pipeline roda ponta a ponta', () => {
  it('as três cadeias completam todas as trajetórias sem NaN', () => {
    // Este é o portão de entrada do Sprint 8 que de fato importa: se o
    // pipeline não roda, não há o que medir com humano no Dia 7.
  ...
  it('o resultado DIZ o que mediu', () => {
    ...
    expect(r.featureSet).toBe(EXPERIMENT.featureSet);
  });
});
...
describe('a troca de filtro preservou o comportamento default', () => {
  it("`oneEuro` reproduz o baseline com ZERO regressões", () => {
    ...
    // no caminho que roda hoje. Se este teste falhar, a refatoração do `P7.6`
    // mudou o produto — e todo baseline gravado antes dela deixa de valer.
...
describe('kalmanEma NÃO passa no aceite — e a medição fica registrada', () => {
  it('regride sobre as trajetórias sintéticas, e isso é reportado, não escondido', () => {
    ...
    // do Sprint 8 num carimbo. A tolerância existe para detectar regressão;
  ...
  it('e por isso o harness NÃO pode decidir sobre o kalmanEma', () => {
    ...
    //
    // Quem decide é o `F8.5`, com humano e com o pipeline real. O que o `P7.6`
    // entrega é a possibilidade de fazer a comparação, que antes não havia.
    //
...
describe('as flags do Sprint 7 não mudam o baseline enquanto estão desligadas', () => {
  it('todas nascem no comportamento atual', () => {
    // O aceite pede o harness "com as flags novas ligadas". Elas são
    // selecionáveis (é o que o sprint entrega), mas o DEFAULT continua sendo o
    // produto de hoje: ligar uma flag é uma decisão de medição, não um efeito
    // colateral de instalá-la.
    ...
    expect(EXPERIMENT.gazeLostFallback).toBe(false);
  });
