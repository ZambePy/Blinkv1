...
// -----------------------------------------------------------------------------
// B3.16 — `evaluateReadiness` e `aggregateSnapshots` não tinham chamador de
//         produção.
//
// `grep` por essas duas funções retornava apenas `setupReadiness.ts` e
// `setupReadiness.test.ts`. O módulo inteiro estava escrito, testado e
// correto — e nunca rodava.
//
// O README destaca "verificação de prontidão ao vivo — distância,
// enquadramento, postura da cabeça, iluminação, contraste, reflexo em lentes e
// cintilação, todos medidos e exibidos antes de gastar tempo de coleta com
// dado ruim". Nada disso chegava ao cuidador.
//
// O que se perdia, em concreto:
//   • a checagem de VIEWPORT — única defesa contra o erro angular inflado por
//     rodar em janela não-maximizada (ver B2.9 e B2.10);
//   • a avaliação de `distanceRange`;
//   • o aviso de cintilação;
//   • e o `RunMeta` do relatório voltava a ser digitado à mão em vez de vir de
//     medição.
//
// A peça que faltava era a CONVERSÃO: `EngineDiagnostics` e
// `ReadinessSnapshot` têm formatos diferentes e ninguém tinha escrito a ponte.
// -----------------------------------------------------------------------------
...
function diagnostics(over: Partial<EngineDiagnostics> = {}): EngineDiagnostics {
  return {
    ...
    experiment: { expandFactor: 1.4, cadenceMs: 100, applyGazeCorrection: false },
    framing: {
      hasFace: true,
      iod: 0.12,
...
describe('B3.16 — a ponte entre o engine e a prontidão existe', () => {
  ...
  it('o snapshot alimenta `evaluateReadiness` sem adaptação extra', () => {
    // O teste que fecha o buraco: a saída do adaptador é aceita direto pela
    // função que nunca tinha sido chamada.
    const s = snapshotFromDiagnostics(diagnostics(), VIEWPORT)!;
...
describe('B3.16 — qualidade não medida não vira veredito', () => {
  it('brightness undefined devolve null, não um snapshot com zeros', () => {
    // Herda a disciplina de B3.3: `undefined` é "não medido". Preencher com
