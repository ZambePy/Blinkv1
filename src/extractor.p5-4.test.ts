...
// -----------------------------------------------------------------------------
// P5.4 — EAR normalizado e o limiar 0,18 (conflito C7).
//
// A especificação pede `threshold = 0.18` para olho fechado. Isso só passou a
// ser possível depois de `B2.6` isotropizar o EAR: num EAR de escala 0,55
// (anisotrópico) o valor 0,18 nunca dispararia.
//
// ── Onde o 0,18 entra, e por que não como limiar único ──────────────────────
//
// Um limiar ABSOLUTO único seria perigoso justamente para o público-alvo. Uma
// pessoa com ptose — comum em ELA — pode ter EAR de repouso em 0,20; um corte
// fixo em 0,18 deixaria a margem de detecção em 0,02 e transformaria tremor de
// landmark em piscada. É para isso que existe o limiar ADAPTATIVO, que aprende
// o repouso da pessoa.
//
// O 0,18 entra onde ele de fato significa "olho fechado sem depender de quem":
// no BOOTSTRAP, antes de haver histórico para adaptar.
// -----------------------------------------------------------------------------

describe('as constantes na escala isotrópica', () => {
  it('0,18 é o limiar absoluto de olho fechado (C7)', () => {
  ...
  it('o limiar adaptativo do repouso mediano cai dentro da faixa', () => {
    ...
    // adaptação e o `blinkRatio` deixa de ter efeito (foi o bug de B2.6).
...
describe('bootstrap — o detector não pode travar antes de aprender', () => {
  it('REGRESSÃO: repouso abaixo de thrMax não trava o detector', () => {
    // O bug medido: `thr = thrMax` (0,28) durante o bootstrap, e o histórico só
    // acumula em quadros SEM piscada. Uma pessoa com repouso 0,25 tinha TODOS
    // os quadros marcados como piscada, o histórico nunca enchia, e o limiar
    // ficava em 0,28 para sempre. Medido: 200 de 200 quadros vinham como
    // piscada, e `restingEar` ficava em null indefinidamente.
    //
    // Efeito no produto: o app liga, roda, e nunca rastreia — para essa pessoa.
    // E repouso baixo (ptose) é comum em ELA, que é o público-alvo.
...
// -----------------------------------------------------------------------------
// Invariância à resolução — o aceite explícito de P5.4.
// -----------------------------------------------------------------------------
...
describe('invariância à resolução (aceite de P5.4)', () => {
  it('o MESMO olho físico dá o MESMO EAR em 1080p e em 720p', () => {
    ...
    // justamente isso que a correção de anisotropia de `B2.6` desfaz.
