...
//
// O defeito (tasks.md §Sprint 1): `submitTensor` só olhava a cadência.
//
//   if (now - lastSubmitMs < cadenceMs) return false;   // única condição
//   const id = ++pendingId;                             // nunca comparado
//
// Nada olhava quantas inferências estavam em voo. Com `l2csCadenceMs = 100` são
// 10 submissões/s, mas o worker roda WASM single-thread (`numThreads = 1`) e
// ResNet-50 @448² ≈ 16 GFLOPs custa 300–600 ms no browser. A ~3 inferências
// consumidas contra 10 produzidas sobram ~7 mensagens/s na fila, cada uma
// segurando 3·448·448·4 = 2.408.448 B ≈ 2,3 MiB → **~16 MiB/s monotônicos**.
// Em 60 s de calibração: ~1 GB, aba morta por OOM ou swap travando o rAF.
//
// Correção: contador de in-flight; `canSubmit()` false enquanto `pending > 0`;
// decrementa em `result` E em `infer_error` (senão um único erro trava o
// pipeline para sempre); `pendingCount` exposto no diagnóstico.
// -----------------------------------------------------------------------------
...
describe('B1.2 — backpressure: nunca mais de uma inferência em voo', () => {
  ...
  it('canSubmit() é false enquanto há inferência em voo', async () => {
    ...
    expect(client.canSubmit(10_000)).toBe(false);

    worker.deliver({ type: 'result', id: 1, yaw: 0.1, pitch: -0.1, confidence: 0.8, inferenceMs: 400 });
    expect(client.canSubmit(10_000)).toBe(true);
  });
  ...
  it('100 tentativas contra um worker mudo produzem exatamente 1 submissão', async () => {
    ...
    // ANTES da correção: 100 mensagens na fila = ~230 MiB retidos.
    expect(worker.inferCount()).toBe(1);
  });
...
describe('B1.2 — o slot é liberado também em erro', () => {
...
describe('B1.2 — a taxa de submissão converge para a taxa de consumo', () => {
  it('com latência 500 ms e cadência 100 ms, submete ~1 a cada 500 ms, não 10', async () => {
    // Reproduz o cenário do bug com relógio controlado: o worker leva 500 ms,
    // a cadência permitiria 10 submissões/s. Sem backpressure seriam ~20
    // submissões em 2 s (≈46 MiB retidos); com backpressure, ~4.
    let agora = 0;
